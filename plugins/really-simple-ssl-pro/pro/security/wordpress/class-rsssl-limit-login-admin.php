...
use REALLY_SIMPLE_SSL\Security\WordPress\Limitlogin\Rsssl_Login_Data_Manager;
...
class Rsssl_Limit_Login_Admin {
	...
	public function __construct() {
		...
		add_action( 'rsssl_install_tables', array( $this, 'create_login_attempts_table' ) );
	...
	public function handle_entities( array $data, string $type = 'source_ip' ): array {
		...
		foreach ( $data['values'] as $key => $entry ) {
			$newData               = $data;
			$newData['value']      = $entry;
			$entry_value           = $this->sanitize_text_field( $data, 'value', $type );
			$data['value'][ $key ] = $entry_value;
			unset( $newData );
		}
		$errors = array();
		foreach ( $data['value'] as $value_loop ) {
			$entry_exists = $this->check_entry( $wpdb, $type, $value_loop );

			if ( $entry_exists > 0 ) {
				$this->delete_existing_entry( $wpdb, $type, $value_loop );
			}

			try {
				if ( 'region' === $type ) {
					// we get all countries and then we add them to the allowlist.
					$countries = $this->continent_to_iso2country( $value_loop );
					foreach ( $countries as $country ) {
						// we check if the country already exists. if not we add it.
						$country_exists = $this->check_entry( $wpdb, 'country', $country );
						if ( $country_exists > 0 ) {
					...
					$this->insert_data_into_database( $wpdb, $value_loop, $type, $entry_status );
				}
			} catch ( Exception $e ) {
	...
	public function delete_entries( array $data, bool $region = false ): array {
		...
		if ( empty( $ids ) ) {
			return $this->json_response( false, __( 'No records to delete.', 'really-simple-ssl' ) );
		}
